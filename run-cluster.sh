...
kill_servers() {
    echo "Killing all running hastacian2 servers..."
...
run_servers() {
    echo "Building hastacian2..."
    cargo build

    echo "Killing any existing hastacian2 servers..."
    kill_servers

    sleep 1

    echo "Starting 3 hastacian2 servers..."
    echo ""

    # Start servers in background, writing to both log files and stdout
    ./target/debug/hastacian2 --id 1 --http-addr 127.0.0.1:21001 2>&1 | tee n1.log &
    sleep 1
    echo "Server 1 started on port 21001"

    ./target/debug/hastacian2 --id 2 --http-addr 127.0.0.1:21002 2>&1 | tee n2.log &
    sleep 1
    echo "Server 2 started on port 21002"

    ./target/debug/hastacian2 --id 3 --http-addr 127.0.0.1:21003 2>&1 | tee n3.log &
    sleep 1
