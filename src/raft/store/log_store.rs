    ...
    C: RaftTypeConfig,
{
    ...
    /// Get store metadata.
    ///
    /// It returns `None` if the store does not have metadata stored.
    fn get_meta<M: StoreMeta<C>>(&self) -> Result<Option<M::Value>, io::Error> {
        ...
        let t = rmp_serde::from_slice(&bytes)
    ...
    fn put_meta<M: StoreMeta<C>>(&self, value: &M::Value) -> Result<(), io::Error> {
        ...
        self.db
            .put_cf(self.cf_meta(), M::KEY, encoded_value)
            .map_err(|e| io::Error::other(e.to_string()))?;
    ...
    C: RaftTypeConfig,
{
    async fn try_get_log_entries<RB: RangeBounds<u64> + Clone + Debug + OptionalSend>(
        ...
        for item_res in it {
            ...
            let entry: EntryOf<C> = rmp_serde::from_slice(&val).map_err(read_logs_err)?;
    ...
    async fn get_log_state(&mut self) -> Result<LogState<C>, io::Error> {
        ...
        let last_log_id = match last {
            ...
            Some(res) => {
                ...
                let ent =
                    rmp_serde::from_slice::<EntryOf<C>>(&entry_bytes).map_err(read_logs_err)?;
                Some(ent.log_id())
            }
        };
